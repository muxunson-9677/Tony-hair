...
test('completes the V3 choose, compare, barber, and adjustment journey on a phone', async ({ context, page }, testInfo) => {
  ...
  try {
    ...
    await expect(page.getByRole('group', { name: '这次你最想解决什么？' })).toBeVisible()
    ...
    await expect(page.getByText('现在能不能剪')).toHaveCount(2)
    await expect(page.getByText('每天打理')).toHaveCount(2)
    await expect(page.getByText('变化程度')).toHaveCount(2)
    await expect(page.getByText('最大风险')).toHaveCount(2)
    ...
    await page.getByLabel('备选方案').selectOption({ label: '纹理短碎发' })
    await page.getByRole('button', { name: '保存沟通卡' }).click()
    await expect(page.getByText('备选 · 纹理短碎发')).toBeVisible()
